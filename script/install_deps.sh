...
install_osqp_eigen() {
    ...
    if ( ls /usr/local/include | grep osqp);then
        ...
        cd $TEMP_DIR
        git clone --recursive https://github.com/oxfordcontrol/osqp --branch release-0.6.3
        cd osqp
        mkdir build && cd build
...
install_grid_map() {
    sudo apt-get -y install ros-melodic-pcl-ros ros-melodic-costmap-2d ros-melodic-grid-map
